...
#Raw data before cutoff

# Read and prepare the dataset
multispq_densityplot <- fread("Nebraska2020MultiSpeQPostFlowering.csv")
...
# Define cutoffs for each trait
cutoffs <- data.frame(
  Trait = c('Relative_Chlorophyll', 'qL', 'PS1_Active.Centers', 'vH.', 'NPQt',
            'FvP_over_FmP', 'PhiNO', 'gH.', 'Phi2', 'PhiNPQ', 'ECS_tau',
            'PS1_Oxidized.Centers', 'PS1_Over.Reduced.Centers', 'PS1_Open.Centers'),
  Lower = c(38, 0, 0, 0, 0, 0.35, 0, 0, 0.2, 0, 0, -2, -2, -2),
  Upper = c(100, 0.8, 9, 0.3, 7, 0.85, 0.36, 500, 0.7, 0.7, 0.02, 3, 2, 1.5)
)
...
for (trait in raw_trait_names) {  # Loop over raw_trait_names
  ...
  # Extract cutoffs for the current trait
  current_cutoffs <- cutoffs %>% filter(Trait == trait)
  
  plot <- ggplot(multispq_densityplot, aes(x = .data[[trait]])) +
    geom_density(fill = colors_for_traits[trait], alpha = 0.7) +
    geom_vline(xintercept = current_cutoffs$Lower, color = "red", linetype = "dashed") +  # Add lower cutoff
    geom_vline(xintercept = current_cutoffs$Upper, color = "red", linetype = "dashed") +  # Add upper cutoff
...
ggsave("combined_density_plots_with_cutoffs.svg", plot = combined_plot, width = 18, height = 12, dpi = 300)
...
setwd("~/Documents/PhD work/Photosyntheiss_project/Photosynthsis_paper_data/Raw_data")
